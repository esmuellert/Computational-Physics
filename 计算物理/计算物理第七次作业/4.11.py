...
class trace(object):
    def __init__(self,x_0,y_0,v_0,a_f,beta,time_step=0.01,time_total=100):
        self.v_x0 = 0
        self.a_f=0.1
        self.v_y0 = v_0
        self.x_0 = x_0
    ...
    def calculate(self):
        for i in range(self.t.shape[0] - 1):
            self.v_x[i+1] = -4*math.pi**2*self.x[i]/(self.r[i]**(self.beta+1))*self.time_step*(1+self.a_f/(self.r[i]**2)) + self.v_x[i]
            self.v_y[i+1] = -4*math.pi**2*self.y[i]/(self.r[i]**(self.beta+1))*self.time_step*(1+self.a_f/(self.r[i]**2)) + self.v_y[i]
...
start = time.time()
a = trace(x_0=1.5,y_0=0,v_0=2*math.pi,a_f=0.1,beta=2.05)
a.calculate()
a.plot()
end = time.time()
end-start