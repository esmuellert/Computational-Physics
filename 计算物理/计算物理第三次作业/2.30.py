...
class ball(object):
    ...
    def calculate_with_drag(self):
        for i in range(self.t.shape[0]-1):
            ...
            self.v_y[i+1] = self.v_y[i] + 0.423*math.sin(20*math.pi/180)*self.g*self.time_step - self.g * self.time_step - self.B2_sub_m*math.sqrt(self.v_x[i]**2+self.v_y[i]**2+self.v_z[i]**2)*self.v_y[i]*self.time_step
            self.y[i+1] = self.y[i] + self.v_y[i] * self.time_step
            self.v_z[i+1] = self.v_z[i] + 0.423*math.cos(20*math.pi/180)*self.g*self.time_step-self.B2_sub_m*math.sqrt(self.v_x[i]**2+self.v_y[i]**2+self.v_z[i]**2)*self.v_z[i]*self.time_step
...
baseball = ball(v_0=40,theta=0,time_duration=1)
